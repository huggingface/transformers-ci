...
Driven by `.github/workflows/assign-reviewers.yml` in this repo, which repositories call as a
reusable workflow; exposed as the `assign-reviewers` console script. The reviewed repository is
checked out in the working directory and supplies its own codeowners file — see
`transformersci.reviewers.resolver` for how a file is resolved to its owners.

Runs from the caller's `pull_request_target`, so it has a write token and reads some files at the
PR's head commit (`read_head_file`) to see models and `# Reviewers:` tags the PR adds. Nothing
from the head is ever checked out or executed: the fetched content is only parsed as text, and
`is_collaborator` gates every name before a review is requested, so a `# Reviewers:` tag in an
untrusted PR cannot be used to ping arbitrary accounts. The codeowners file itself is only ever
read from the checked-out base, never from the head, so a PR cannot re-route its own review; the
caller's PR security gate blocks untrusted PRs from touching it as well.
"""
...
def request_reviews(repo, pr, candidates, limit=MAX_REVIEWERS):
    ...
    for login in candidates:
        ...
        if not is_collaborator(repo, login):
            warn(
                f"Skipping {login}: not a collaborator of {repo.full_name} (stale codeowners entry?)"
            )
            continue
        try:
...
def main():
    # The reviewed repository is checked out in the working directory and owns this file. Always
    # the base version: a PR must not be able to re-route its own review by editing it.
    codeowners_path = os.environ.get("CODEOWNERS_FILE") or CODEOWNERS_PATH
    codeowners_lines = (
        Path(codeowners_path).read_text(encoding="utf-8").splitlines(keepends=True)
    )
    ...
    print(
        ...
        [
            (spelling[key], locs)
            for key, locs in locs_per_owner.most_common(MAX_REVIEWERS)
        ],
    )
