...
"""A shared module to determine who should review a given file, using the reviewed repo's
`.github/scripts/codeowners_for_review_action`.

The codeowners file stays in the repository being reviewed — who owns what is that repo's
decision — and only the resolution logic lives here. Shared by the reviewer-assignment workflow
(`transformersci.reviewers.assign`, which resolves against the PR being opened) and by the
repository check (`utils/check_reviewers.py` in huggingface/transformers, which resolves against
the whole tree). Reading files is injected so both can use their own source: the workflow prefers
the PR head for files it touches, the check reads the working tree.
...
def pr_author_is_in_hf(pr_author, codeowners_lines):
    ...
    return any(
        author in {owner.casefold() for owner in owners}
        for _, owners in iter_rules(codeowners_lines)
    )
...
def modality_owners_table(codeowners_lines):
    ...
    return {
        ...
        if pattern.startswith(MODALITY_PREFIX)
        and pattern.removeprefix(MODALITY_PREFIX) in known
    }
...
def unknown_modality_slugs(codeowners_lines):
    ...
    return [
        ...
        if pattern.startswith(MODALITY_PREFIX)
        and pattern.removeprefix(MODALITY_PREFIX) not in known
    ]
...
def modality_of_model(model, read_file=read_local_file):
    ...
    candidates = [
        (len(page), modality)
        for page, modality in index.items()
        if len(page) >= MIN_DOC_PREFIX and key.startswith(page)
    ]
    if not candidates:
        return None
