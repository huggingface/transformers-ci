...
class TrackingIssueTest(unittest.TestCase):
    ...
    def test_issue_body_renders_table(self):
        ...
        self.assertIn("| Model | Error | Occurrences | PR | Task |", body)
        self.assertIn("| --- | --- | --- | --- | --- |", body)
    ...
    def test_task_column_links_serge_with_a_padlock(self):
        ...
        body = itf.render_tracking_issue_body(
            ...
            ["2026-06-19"],
            "2026-06-19",
            job_ids={fp: "854dd091bfd3403281fea7e41e61a946"},
            serge_url="https://serge.huggingface.tech/",
            task_repo="huggingface/transformers",
        )
        self.assertIn(
            "[🔒 task](https://serge.huggingface.tech/tasks/huggingface/"
            "transformers/854dd091bfd3403281fea7e41e61a946)",
            body,
        )
        # The padlock has to be explained, or it reads as a broken link: the
        # host is VPN-internal and the page needs a Serge login.
        self.assertIn("VPN-internal", body)

    def test_task_column_is_a_dash_before_serge_accepts_the_dispatch(self):
        # The issue is rendered once before any task exists. A link to a job id
        # that was never minted is worse than no link.
        targets = [self._target("g1", "alpha")]
        body = itf.render_tracking_issue_body(targets, ["2026-06-19"], "2026-06-19")
        self.assertIn("| Task |", body)
        row = next(ln for ln in body.splitlines() if "`alpha`" in ln)
        self.assertTrue(row.rstrip().endswith("| — |"), row)
        self.assertNotIn("🔒 task", body)

    def test_task_column_is_a_dash_when_a_group_got_no_job_id(self):
        # Dispatch refused (e.g. an expired OIDC bearer): the other groups link,
        # this one must not borrow someone else's job.
        targets = [self._target("g1", "alpha"), self._target("g2", "beta")]
        fp0 = itf.target_fingerprint(targets[0])
        body = itf.render_tracking_issue_body(
            targets,
        ...
        self.assertEqual(body.count("🔒 task"), 1)

    def test_a_row_carried_from_a_body_without_the_task_column_still_carries(self):
        """The regression the new column could have caused.

        `_carry_forward_rows` read the PR cell as the LAST cell. With a trailing
        Task column, a row from a pre-column issue body has four cells and a new
        one has five — reading the last cell would test the Task link for
        `#`/🚫/⚠️ and silently drop every resolved row from the table.
        """
        prior_body = "\n".join(
            [
                "| Model | Error | Occurrences | PR |",
                "| --- | --- | --- | --- |",
                "| `convnextv2` | output_mismatch | 2 | #47540 |",
                "",
            ]
        )
        carried = itf._carry_forward_rows(prior_body, [self._target("g3", "helium")])
        self.assertEqual(len(carried), 1)
        self.assertIn("#47540", carried[0])
        # Padded to the current column count so it lands under the right headers.
        self.assertEqual(carried[0].count("|"), 6)
